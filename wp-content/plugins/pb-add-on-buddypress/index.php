<?php
/**
 * Plugin Name: Profile Builder - BuddyPress Add-on
 * Plugin URI: http://www.cozmoslabs.com/wordpress-profile-builder
 * Description: Integrates Profile Builder with BuddyPress, allowing you to transfer BuddyPress user profile to Profile Builder user profile automatically.
 * Version: 1.0.3
 ...
 * @return null
 */
function wppb_bdp_remove_pb_settings_from_subsites(){
    if ( ! function_exists( 'is_plugin_active_for_network' ) ) {
        require_once( ABSPATH . '/wp-admin/includes/plugin.php' );
    }

    $pb_is_network_active = false;
    $pb_plugins = apply_filters( 'wppb_bdp_pb_plugin_files', array( 'profile-builder-pro/index.php', 'profile-builder-hobbyist/index.php', 'profile-builder/index.php', 'profile-builder-2.0/index.php' ) );
    ...
    if ( ! is_main_site( get_current_blog_id() ) && is_plugin_active_for_network( 'buddypress/bp-loader.php' ) && is_plugin_active_for_network( 'pb-add-on-buddypress/index.php' ) && $pb_is_network_active ){
        // disable PB for this site
        remove_action( 'plugins_loaded', 'wppb_plugin_init' );
    }
}
if ( is_multisite() ) {
    add_action('plugins_loaded', 'wppb_bdp_remove_pb_settings_from_subsites', 9);
}
...
function wppb_bdp_plugin_init() {
    ...
    if( function_exists( 'wppb_return_bytes' ) ) {
         ...
         * Define plugin path
         */
        define('WPPB_BDP_PLUGIN_DIR', WP_PLUGIN_DIR . '/' . dirname(plugin_basename(__FILE__)));
        define('WPPB_BDP_PLUGIN_URL', plugin_dir_url(__FILE__) );

        /*
         * Define constants
         */
        define('WPPB_BDP_VISIBILITY_OPTION_NAME', 'wppb_profile_visibility_levels');


        /*
         * Include the file for creating the BuddyPress subpage under Profile Builder menu
         */
        if (file_exists(WPPB_BDP_PLUGIN_DIR . '/buddypress-page.php'))
            include_once(WPPB_BDP_PLUGIN_DIR . '/buddypress-page.php');

        /*
         * Include the file for creating the BuddyPress subpage that handles BP field import
         */
        if (file_exists(WPPB_BDP_PLUGIN_DIR . '/import-bp-fields.php'))
            include_once(WPPB_BDP_PLUGIN_DIR . '/import-bp-fields.php');


        /*
         * Include the DOM parsing library
         */
        if (file_exists(WPPB_BDP_PLUGIN_DIR . '/assets/lib/simple_html_dom.php'))
            include_once(WPPB_BDP_PLUGIN_DIR . '/assets/lib/simple_html_dom.php');
        ...
        *
        */
        if (file_exists(WPPB_BDP_PLUGIN_DIR . '/update/update-checker.php')) {
            include_once(WPPB_BDP_PLUGIN_DIR . '/update/update-checker.php');

            $localSerial = get_option('wppb_profile_builder_pro_serial');

            $wppb_bdp_update = new wppb_PluginUpdateChecker('http://updatemetadata.cozmoslabs.com/?localSerialNumber=' . $localSerial . '&uniqueproduct=CLPBBDP', __FILE__, 'wppb-bdp-add-on');
        }
        ...
        if ( ( in_array( 'buddypress/bp-loader.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) || ( is_plugin_active_for_network('buddypress/bp-loader.php') ) )  {
             ...
             * @return null
             */
            function wppb_bdp_init_translation(){
                $current_theme = wp_get_theme();
                if( !empty( $current_theme->stylesheet ) && file_exists( get_theme_root().'/'. $current_theme->stylesheet .'/local_pb_lang' ) )
                    load_plugin_textdomain( 'profile-builder-buddypress-add-on', false, basename( dirname( __FILE__ ) ).'/../../themes/'.$current_theme->stylesheet.'/local_pb_lang' );
                else
                    load_plugin_textdomain( 'profile-builder-buddypress-add-on', false, basename(dirname(__FILE__)) . '/translation/' );
            }
            add_action('init', 'wppb_bdp_init_translation', 8);
             ...
             * @return null
             */
            function wppb_bdp_scripts_and_styles_front_end() {
                wp_enqueue_style('wppb-buddypress-integration-frontend-style', WPPB_BDP_PLUGIN_URL . 'assets/css/wppb-bdp-style-front-end.css');
                global $wppb_shortcode_on_front;
                if( !empty( $wppb_shortcode_on_front ) && $wppb_shortcode_on_front === true ) {
                    wp_enqueue_script( 'wppb-bdp-front-end', WPPB_BDP_PLUGIN_URL . '/assets/js/wppb-bdp-front-end.js', array( 'jquery' ) );
                }
            }
            add_action( 'wp_footer', 'wppb_bdp_scripts_and_styles_front_end' );
             ...
             *
             */
            function wppb_bdp_scripts_and_styles_admin( $hook ) {
                if ( 'profile-builder_page_manage-fields' == $hook && ! isset( $_GET['wppb_rpf_repeater_meta_name'] ) ){
                    wp_enqueue_script( 'wppb-bdp-integration', WPPB_BDP_PLUGIN_URL . '/assets/js/wppb-bdp-admin.js', array( 'jquery' ), false, true );
                }
                if ( ( 'user-edit.php' == $hook ) || ( 'profile.php' == $hook ) || ( 'post.php' == $hook ) || ( 'profile-builder_page_profile-builder-buddypress' == $hook ) ) {
                    wp_enqueue_style('wppb-buddypress-integration-backend-style', WPPB_BDP_PLUGIN_URL . 'assets/css/wppb-bdp-style-back-end.css');
                }
            }
            add_action( 'admin_enqueue_scripts', 'wppb_bdp_scripts_and_styles_admin', 20 );

            /*
             * Include the file managing Field Visibility features
             */
            if (file_exists(WPPB_BDP_PLUGIN_DIR . '/field-visibility.php'))
                include_once(WPPB_BDP_PLUGIN_DIR . '/field-visibility.php');

            /*
             * Include the file managing mustache vars for Userlisting and Email Customizer
             */
            if (file_exists(WPPB_BDP_PLUGIN_DIR . '/mustache-vars.php'))
                include_once(WPPB_BDP_PLUGIN_DIR . '/mustache-vars.php');
             ...
             * @return null
             */
            function wppb_bdp_remove_pb_avatar(){
                remove_filter( 'wppb_output_form_field_avatar', 'wppb_avatar_handler', 10 );
            }
            add_filter( 'wppb_output_form_field_avatar', 'wppb_bdp_remove_pb_avatar', 5 );

            function wppb_bdp_remove_pb_avatar_from_backend(){
                remove_filter( 'wppb_admin_output_form_field_avatar', 'wppb_avatar_handler', 10 );
            }
            add_filter( 'wppb_admin_output_form_field_avatar', 'wppb_bdp_remove_pb_avatar_from_backend', 5 );

            function wppb_bdp_remove_pb_avatar_save(){
                remove_action( 'wppb_backend_save_form_field', 'wppb_save_avatar_value', 10 );
            }
            add_action( 'wppb_save_form_field', 'wppb_bdp_remove_pb_avatar_save', 5 );

            function wppb_bdp_remove_pb_avatar_save_from_backend(){
                remove_action( 'wppb_save_form_field', 'wppb_save_avatar_value', 10 );
            }
            add_action( 'wppb_save_form_field', 'wppb_bdp_remove_pb_avatar_save', 5 );

            function wppb_bdp_remove_pb_avatar_check(){
                remove_filter( 'wppb_check_form_field_avatar', 'wppb_check_avatar_value', 10 );
            }
            add_filter( 'wppb_check_form_field_avatar', 'wppb_bdp_remove_pb_avatar_check', 5);
             ...
             * @return string $form
             */
            function wppb_manage_fields_display_avatar_notice( $form ){
                // add a notice to 'Avatar' field
                global $wppb_results_field;
                if ( $wppb_results_field == 'Avatar' )
                    $form .= '<div id="wppb-avatar-nag" class="wppb-backend-notice">' . __( 'Profile Builder Avatar field is disabled to allow use of BuddyPress Avatar.', 'profile-builder-buddypress-add-on' ) . '</div>';

                return $form;
            }
            add_filter( 'wck_after_content_element', 'wppb_manage_fields_display_avatar_notice' );
             ...
             */
            function wppb_bdp_get_template_directory(){
                return WPPB_BDP_PLUGIN_DIR . '/templates';
            }

            if ( function_exists('bp_register_template_stack') ) {
                bp_register_template_stack('wppb_bdp_get_template_directory', 10);
            }
            ...
            add_filter( 'bp_get_template_part' , 'wppb_bdp_filter_template_parts', 10, 3);
             ...
             * @return null
             */
            function wppb_bdp_update_last_activity_usermeta( $user_id, $time ){
                update_user_meta( $user_id, 'wppb_bdp_last_activity', $time );
            }
            add_action( 'bp_core_user_updated_last_activity', 'wppb_bdp_update_last_activity_usermeta', 10, 2 );
             ...
             * @return null
             */
            function wppb_bdp_redirect_to_manage_fields(){
                global $pagenow;
                ...
                if ( $pagenow == 'users.php' && isset( $_GET['page'] ) && array_key_exists  ( esc_attr( $_GET['page'] ), $redirect_array ) ) {
                    wp_safe_redirect( $redirect_array[ esc_attr ( $_GET['page'] ) ] );
                }
            }
            add_action( 'admin_init', 'wppb_bdp_redirect_to_manage_fields' );
             ...
             * @return null
             */
            function wppb_bdp_remove_manage_signups_when_ec_off(){
            ...
            add_action( 'admin_menu', 'wppb_bdp_remove_manage_signups_when_ec_off', 999 );
             ...
             * @return null
             */
            function wppb_bdp_admin_notice() {
                ?>
                <div class="update-nag">
                    <?php _e( 'BuddyPress needs to be installed and activated for Profile Builder - BuddyPress Integration Add-on to work as expected!', 'profile-builder-buddypress-add-on' ); ?>
                </div>
                <?php
            }
            add_action( 'admin_notices', 'wppb_bdp_admin_notice' );
        }
    }
}
add_action( 'plugins_loaded', 'wppb_bdp_plugin_init', 11 );


/**
 * Register activation hook to set default add-on settings if not present
 *
 * @since 1.0.0
 *
 * @return null
 */
function wppb_bdp_set_default_settings_on_activation() {
    wppb_buddypress_settings_defaults();
}
register_activation_hook( __FILE__, 'wppb_bdp_set_default_settings_on_activation' );
 ...
 * @return null
 */
function wppb_buddypress_settings_defaults(){
    $wppb_buddypress_settings = get_option( 'wppb_buddypress_settings', 'not_found' );
    ...
    if ( $wppb_buddypress_settings == 'not_found' ) {
        ...
        wppb_bdp_set_default_userlisting_templates( $ul_post_id );
 ...
 * @return null
 */
function wppb_bdp_set_default_userlisting_templates( $ul_post_id ){
    $ul_post_id = (int) $ul_post_id;
    update_post_meta( $ul_post_id, 'wppb-ul-templates', wppb_bdp_generate_allUserlisting_content() );
    update_post_meta( $ul_post_id, 'wppb-single-ul-templates', wppb_bdp_generate_singleUserlisting_content() );
 ...
 */
function wppb_bdp_generate_allUserlisting_content(){
 ...
 */
function wppb_bdp_generate_singleUserlisting_content(){
    return '
<ul class="wppb-profile">
  <li>
    <h3>' . __( 'Name', 'profile-builder-buddypress-add-on' ) . '</h3>
  </li>
  <li>
    <label>' . __( 'Username:', 'profile-builder-buddypress-add-on' ) . '</label>
    <span>{{meta_user_name}}</span>
  </li>
  <li bp_visibility="first_name" user_id="{{user_id}}">
    <label>' . __( 'First Name:', 'profile-builder-buddypress-add-on' ) . '</label>
    <span>{{meta_first_name}}</span>
  </li>
  <li bp_visibility="last_name" user_id="{{user_id}}">
    <label>' . __( 'Last Name:', 'profile-builder-buddypress-add-on' ) . '</label>
    <span>{{meta_last_name}}</span>
  </li>
  <li bp_visibility="nickname" user_id="{{user_id}}">
    <label>' . __( 'Nickname:', 'profile-builder-buddypress-add-on' ) . '</label>
    <span>{{meta_nickname}}</span>
  </li>
  <li>
    <label>' . __( 'Display name:', 'profile-builder-buddypress-add-on' ) . '</label>
	<span>{{meta_display_name}}</span>
  </li>
  <li bp_visibility="website" user_id="{{user_id}}">
    <h3>' . __( 'Contact Info', 'profile-builder-buddypress-add-on' ) . '</h3>
  </li>
  <li bp_visibility="website" user_id="{{user_id}}">
  	<label>' . __( 'Website:', 'profile-builder-buddypress-add-on' ) . '</label>
	<span>{{meta_website}}</span>
  </li>
  <li bp_visibility="biographical_info" user_id="{{user_id}}">
    <h3>' . __( 'About Yourself', 'profile-builder-buddypress-add-on' ) . '</h3>
  </li>
  <li bp_visibility="biographical_info" user_id="{{user_id}}">
	<label>' . __( 'Biographical Info:', 'profile-builder-buddypress-add-on' ) . '</label>
